...
if (isset($_GET['order_success']) && $_GET['order_success'] == 3) {
    echo "<script>alert('Total Amount must be greater than 150')</script>";
    echo "<script>window.location.assign('cart.php')</script>";
}
                    				...
                    				<table class="table table-bordered">
                    					...
                    					<tbody>
                    						...
                    						for ($i=0; $i < count($_SESSION['cart']); $i++) { 
                    							$select = "SELECT * FROM groceroo_products where product_id = {$_SESSION['cart'][$i]}";
                    							$query = mysqli_query($conn, $select);
                    							$j = $i;
                    							while ($res = mysqli_fetch_assoc($query)) { 
                    						...
                    						<tr>
                    							...
                    							<td><input class="form-control" type="number" min="1" max="10" step="1" value="1" name="product_quantity[]" onchange="prodTotal(this)"></td>
                                            ...
                                            <tr>
                                                ...
                                                <td colspan="3">
                                                    ...
                                                    <label>
                                                        <?php
                                                            if(isset($_SESSION["cardPay"])){
                                                                echo '<input type="radio" name="radio1" value="Card" checked="checked" required="">Card';
                                                            }else{
                                                                echo '<input type="radio" name="radio1" value="Card" required="">Card';
                                                            }
                                                        ?>
