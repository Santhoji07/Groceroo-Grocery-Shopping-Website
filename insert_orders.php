...
if (isset($_SESSION['cart']) && $_SESSION['cart'] !== null) {
    if (isset($_SESSION['user_users_id']) && $_SESSION['user_users_username'] !== null) {
        ...
        if ($total_amount <= 150) {
            ...
            $insert_orders = "INSERT INTO groceroo_orders (users_id, delivery_date, payment_method, total_amount) values ('$users_id', '$delivery_date', '$payment_method', '$total_amount')";
            mysqli_query($conn, $insert_orders);
            $orders_id = mysqli_insert_id($conn);
            for ($i = 0; $i < count($product_name); $i++) {
                $insert_orders_detail = "INSERT INTO groceroo_order_details (orders_id, product_name, quantity) values ('$orders_id', '$product_name[$i]', '$quantity[$i]')";
